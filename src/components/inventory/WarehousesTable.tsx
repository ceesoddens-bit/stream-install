...
type WarehousesColumnDef = {
  key: WarehousesColumnKey;
  label?: string;
  width: number;
  minWidth: number;
  resizable: boolean;
...
const columns: WarehousesColumnDef[] = [
  { key: 'select', width: 56, minWidth: 48, resizable: false, thClassName: 'p-3 px-4' },
  { key: 'shortCode', label: 'Korte code', width: 140, minWidth: 120, resizable: true, thClassName: 'p-3 text-[11px] font-extrabold text-gray-500 uppercase tracking-wider' },
  { key: 'name', label: 'Naam', width: 260, minWidth: 180, resizable: true, thClassName: 'p-3 text-[11px] font-extrabold text-gray-500 uppercase tracking-wider' },
  { key: 'sub', label: 'Su…', width: 90, minWidth: 80, resizable: true, thClassName: 'p-3 text-[11px] font-extrabold text-gray-500 uppercase tracking-wider' },
  { key: 'createdAt', label: 'Gemaakt op', width: 170, minWidth: 150, resizable: true, thClassName: 'p-3 text-[11px] font-extrabold text-gray-500 uppercase tracking-wider' },
  { key: 'createdBy', label: 'Gemaakt door', width: 210, minWidth: 170, resizable: true, thClassName: 'p-3 text-[11px] font-extrabold text-gray-500 uppercase tracking-wider' },
  { key: 'updatedAt', label: 'Bijgewerkt op', width: 170, minWidth: 150, resizable: true, thClassName: 'p-3 text-[11px] font-extrabold text-gray-500 uppercase tracking-wider' },
  { key: 'updatedBy', label: 'Bijgewerkt door', width: 220, minWidth: 170, resizable: true, thClassName: 'p-3 text-[11px] font-extrabold text-gray-500 uppercase tracking-wider' },
...
export function WarehousesTable() {
  ...
  const [columnWidths, setColumnWidths] = useState<Record<WarehousesColumnKey, number>>(() => {
    return columns.reduce((acc, col) => {
      ...
      return acc;
    }, {} as Record<WarehousesColumnKey, number>);
  });
  ...
  const startResize = (key: WarehousesColumnKey) => (e: React.PointerEvent) => {
        ...
        <div className="overflow-auto">
          <table className="w-full text-left border-collapse table-fixed" style={{ minWidth: tableMinWidth }}>
            ...
            <tbody className="divide-y divide-gray-100">
              {filtered.slice(0, pageSize).map((row) => {
                ...
                return (
                  <tr key={row.id} className={cn('transition-colors', isSelected ? 'bg-emerald-50/40' : 'hover:bg-gray-50/60')}>
                    ...
                    <td className="p-3 text-sm text-emerald-700 font-semibold">{row.shortCode}</td>
                    <td className="p-3">
              ...
              {count === 0 && (
                <tr>
                  <td colSpan={columns.length} className="p-12 text-center text-sm text-gray-500">
                    Geen resultaten.
                  </td>
                </tr>
              )}
